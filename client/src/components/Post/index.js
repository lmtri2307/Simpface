...
function Post({ post }) {
    ...
    const handleComment = useCallback(async (newComment) => {
        api.post.commentPost(post._id, newComment)
            .then(res => {
    ...
    const PostElement = <div className={styles.wrapper} onClick={(e) => { e.stopPropagation() }}>
        {showModal &&
    ...
    if (showModal) {
        return (
                <CommentModal hideModal={() => setShowModal(false)}>
                    {PostElement}
                </CommentModal>
        )
    } else {
